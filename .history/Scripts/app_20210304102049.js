...
((core) =>
{
    ...
    function testPassword(){
        ...
        let messageArea = $("#messageArea");
        let userPasswordPattern = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/;
        ...
        $("#password").on("blur", function()
        {
          ...
          {
            $(this).trigger("focus").trigger("select");
            messageArea.show().addClass("alert alert-danger").text("Please enter a valid Password. (6 characters / 1 capital letter)");
          }
          else
          {
              messageArea.removeAttr("class").hide();
